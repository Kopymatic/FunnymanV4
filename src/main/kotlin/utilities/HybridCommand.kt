...
abstract class HybridCommand {
    ...
    open fun reply(event : SlashCommandInteractionEvent, message : String) : WebhookMessageAction<Message> {
        return event.kReply(message)
    }
    open fun reply(event: MessageReceivedEvent, message: String) : MessageAction {
        return event.kReply(message)
    }
